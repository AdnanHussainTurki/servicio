...
async fn hello_then(sock: &std::path::Path, reqs: Vec<Frame>) -> Vec<Frame> {
    let mut frames = vec![Frame::Request {
        id: 0,
        method: "hello".into(),
        params: json!({"token":"secret"}),
    }];
...
async fn add_then_list_reflects_worker() {
    ...
    let replies = hello_then(
        ...
        vec![
            Frame::Request {
                id: 1,
                method: "add_worker".into(),
                params: json!({ "spec": sleeper("q") }),
            },
            Frame::Request {
                id: 2,
                method: "list_workers".into(),
                params: json!({}),
            },
        ],
    )
...
async fn start_then_stop_worker() {
    ...
    let replies = hello_then(
        ...
        vec![
            ...
            Frame::Request {
                id: 2,
                method: "start_worker".into(),
                params: json!({"name":"q"}),
            },
            Frame::Request {
                id: 3,
                method: "stop_worker".into(),
                params: json!({"name":"q"}),
            },
        ],
    )
...
async fn shutdown_stops_worker_children() {
    ...
    assert_eq!(
        size_a, size_b,
        "worker child kept running after shutdown (orphaned)"
    );
}
...
async fn metrics_method_returns_series_for_running_worker() {
    ...
    let replies = hello_then(
        ...
        vec![Frame::Request {
            id: 1,
            method: "metrics".into(),
            params: json!({"worker":"q","since_secs":3600}),
        }],
    )
    .await;
    match &replies[1] {
...
async fn daemon_log_method_returns_log_field() {
    ...
    let replies = hello_then(
        ...
        vec![Frame::Request {
            id: 1,
            method: "daemon_log".into(),
            params: json!({"lines": 10}),
        }],
    )
    .await;
    match &replies[1] {
...
async fn start_group_starts_all_workers_in_group() {
    ...
    let _ = hello_then(
        ...
        vec![
            Frame::Request {
                id: 1,
                method: "add_worker".into(),
                params: json!({"spec": grouped("a","billing")}),
            },
            Frame::Request {
                id: 2,
                method: "add_worker".into(),
                params: json!({"spec": grouped("b","billing")}),
            },
            Frame::Request {
                id: 3,
                method: "add_worker".into(),
                params: json!({"spec": grouped("c","other")}),
            },
        ],
    )
    .await;
    let replies = hello_then(
        ...
        vec![
            Frame::Request {
                id: 1,
                method: "start_group".into(),
                params: json!({"group":"billing"}),
            },
            Frame::Request {
                id: 2,
                method: "list_workers".into(),
...
async fn export_then_import_workers_roundtrips() {
    ...
    let exp = hello_then(
        ...
        vec![Frame::Request {
            id: 1,
            method: "export_workers".into(),
            params: json!({}),
        }],
    )
    .await;
    let workers = match &exp[1] {
        Frame::Response {
            id: 1,
            result: Some(v),
            ..
    ...
    let imp = hello_then(
        ...
        vec![
            Frame::Request {
                id: 1,
                method: "import_workers".into(),
                params: json!({"workers": workers}),
            },
            Frame::Request {
                id: 2,
                method: "list_workers".into(),
...
async fn detect_workers_finds_laravel_in_fixture() {
    ...
    let replies = hello_then(
        ...
        vec![Frame::Request {
            id: 1,
            method: "detect_workers".into(),
            params: json!({"path": proj.to_str().unwrap()}),
        }],
    )
    .await;
    match &replies[1] {
